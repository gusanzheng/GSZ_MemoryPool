...
#ifndef GSZ_MEMORYPOOL_COMMON_H
#define GSZ_MEMORYPOOL_COMMON_H
#include <iostream>
#include <vector>
#include <ctime>
#include <cassert>
...
static const size_t N_FREE_LISTS = 208;   //thread cache和central cache自由链表哈希桶的表大小

static const size_t N_PAGES = 129;       //page cache中哈希桶的个数
...
inline void* SystemAlloc(size_t kpage)
{
    ...
    if (ptr == nullptr) {
        cout << "Virtual memory is not enough!" << endl;
        throw std::bad_alloc();
    }
    return ptr;
}
